...
describe("EthersWalletImplementation", () => {
	...
	it("Should call encrypt wallet with the correct arguments and return it", () => {
		...
		const encrypt_spy = vi.spyOn(
			EthersWallet.getInstance()["ethersWallet"],
			"encrypt"
		) as MockInstance;
	...
	it("Should generate a new Wallet", () => {
		...
		const mock_generateRandom = vi
			.spyOn(ethers.HDNodeWallet, "createRandom")
	...
	it("Should login with given password and jsonWallet", async () => {
		...
		const mock_fromEncryptedJson = vi.mocked(ethers.Wallet.fromEncryptedJson, {
			partial: true,
			deep: true,
		});
		...
		expect(mock_fromEncryptedJson).toHaveBeenCalledWith(
			jsonWalletString,
			dummyPassword
		);
		expect(mock_fromPhrase).toHaveBeenCalledWith(
			dummyTempWallet.mnemonic?.phrase,
			undefined,
			PARENT_PATH
		);
