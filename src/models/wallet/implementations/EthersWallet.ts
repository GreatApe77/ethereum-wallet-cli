...
import { SettingsFs } from "../../settings/implementations/SettingsFs.js";
...
export class EthersWallet implements Wallet {
	...
	public getBalance(providerUrl:string): Promise<bigint> {
		...
		return this.ethersWallet?.connect(new ethers.JsonRpcProvider(providerUrl)).provider?.getBalance(this.getAddress(settings.settings.connectedAccountIndex)) as Promise<bigint>;
	}
