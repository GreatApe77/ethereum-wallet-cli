...
export async function walletAuthRoutine() {
	if (!fs.existsSync(walletDataPath)) {
      ...
      await walletAuthRoutine(); */
			...
			await typePasswordSubRoutine();
		}
	}
}

async function typePasswordSubRoutine() {
    ...
    spinner.success();
    actionFeedback("Decrypted wallet successfully !", "success");
	} catch (error) {
    spinner.error();
    actionFeedback("Invalid password !", "error");
    await typePasswordSubRoutine();
  }
